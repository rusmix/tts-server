...
async function fetchHistory() {
    const response = await fetch(`${BASE_URL}/api/get-history/`);
...
async function playAudio(fileName) {
    const audio = new Audio(`${BASE_URL}/api/get-file/?file_name=${fileName}`);
    audio.play();
}

async function downloadAudio(fileName) {
    const response = await fetch(`${BASE_URL}/api/get-file/?file_name=${fileName}`);
    if (response.ok) {
...
async function convertTextToSpeech() {
    ...
    await fetch(`${BASE_URL}/api/get-audio/`, {
        method: "POST",
        headers: {
...
async function updateSettings(event) {
    ...
    const response = await fetch(`${BASE_URL}/api/send-params/`, {
        method: "POST",
        headers: {
            "Content-Type": "application/json",
        },
